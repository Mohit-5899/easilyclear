...
# ── Bounded concurrency helper (gemma-tutor fork) ─────────────────────────────
# Every upstream `asyncio.gather(...)` call is replaced with `_bounded_gather(...)`
# so we do not fan out hundreds of concurrent LLM calls and trip OpenRouter
# rate limits. Call `set_gather_concurrency(N)` at startup to tune.
_GATHER_SEMAPHORE: asyncio.Semaphore | None = None


def set_gather_concurrency(limit: int) -> None:
    """Set the max number of PageIndex LLM calls running in parallel."""
    global _GATHER_SEMAPHORE
    _GATHER_SEMAPHORE = asyncio.Semaphore(limit)


async def _bounded_gather(*coros, return_exceptions: bool = False):
    sem = _GATHER_SEMAPHORE or asyncio.Semaphore(5)  # default 5
...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None):
    ...
    results = await _bounded_gather(*tasks, return_exceptions=True)
...
def toc_detector_single_page(content, model=None):
    ...
    return safe_json_field(json_content, 'toc_detected', 'no')
...
def check_if_toc_extraction_is_complete(content, toc, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    return safe_json_field(json_content, 'completed', 'no')
...
def detect_page_index(toc_content, model=None):
    ...
    response = llm_completion(model=model, prompt=prompt)
    json_content = extract_json(response)
    return safe_json_field(json_content, 'page_index_given_in_toc', 'no')
...
async def single_toc_item_index_fixer(section_title, content, model=None):
    ...
    response = await llm_acompletion(model=model, prompt=prompt)
    json_content = extract_json(response)
    physical = safe_json_field(json_content, 'physical_index', None)
    return convert_physical_index_to_int(physical) if physical is not None else None
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    results = await _bounded_gather(*tasks)
