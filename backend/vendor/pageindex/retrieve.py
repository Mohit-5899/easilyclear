...
# gemma-tutor fork: PyPDF2 is dropped as a dependency. The only usage in this
# file is the PDF fallback branch of _get_pdf_page_content when doc_info has
# no cached pages. We lazy-import PyPDF2 inside that branch so this module
# imports cleanly without PyPDF2 installed — gemma-tutor always pre-caches
# pages during ingestion so that branch is never hit in practice.
...
def _get_pdf_page_content(doc_info: dict, page_nums: list[int]) -> list[dict]:
    ...
    path = doc_info['path']
    import PyPDF2  # lazy-imported; gemma-tutor normally uses cached pages
